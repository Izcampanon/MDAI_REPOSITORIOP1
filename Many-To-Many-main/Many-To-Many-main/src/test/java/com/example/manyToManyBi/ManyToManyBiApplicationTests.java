...
class ManyToManyBiApplicationTests {
    ...
    void testIzan(){
        ...
        org.junit.jupiter.api.Assertions.assertEquals(2, usuarioRepository.count(), "Debe haber 2 usuarios");
        org.junit.jupiter.api.Assertions.assertEquals(3, direccionRepository.count(), "Debe haber 3 direcciones");

        // Quitar la relación compartida para el usuario 1 (no elimina la entidad Dirección)
        eliminarDireccionCompartida();
        org.junit.jupiter.api.Assertions.assertEquals(3, direccionRepository.count(), "Direcciones deben seguir siendo 3 tras solo quitar relación");

        // Eliminar la dirección no compartida (se borra de la BD)
        eliminarDireccion();
        org.junit.jupiter.api.Assertions.assertEquals(2, direccionRepository.count(), "Debe quedar 2 direcciones tras eliminar una no compartida");

        // Buscar usuario "Luiky" sin usar stream() sobre Iterable
        Usuario luiky = null;
        for (Usuario u : usuarioRepository.findAll()) {
            if ("Luiky".equals(u.getNombre())) {
                luiky = u;
                break;
            }
        }
        org.junit.jupiter.api.Assertions.assertNotNull(luiky, "Usuario Luiky debe existir");

        // Comprobar estado final de Luiky
        List<Direccion> direccionesLuiky = direccionRepository.findByUsuarios(luiky);
        org.junit.jupiter.api.Assertions.assertEquals(0, direccionesLuiky.size(), "Luiky no debe tener direcciones tras las operaciones");
    }
...
//      usuarioRepository.saveAll(List.of(uLuiky,uLidia));
    ...
    */
    ...
    void eliminarDireccionCompartida() {
        ...
        // Paso 2: Elimina la dirección de la colección del usuario.
        u.setDirecciones(direccionRepository.findByUsuarios(u)); //esta en LAZY necesito cargarlas
